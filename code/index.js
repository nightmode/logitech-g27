...
function findWheel() {
    ...
    */
    ...
    for (let i in devices) {
        ...
        // devices[i].product should be set to 'G27 Racing Wheel'.
        ...
        if (devices[i].vendorId === 1133 &&
            (devices[i].productId === 49812 || devices[i].productId === 49819 || devices[i].product === 'G27 Racing Wheel') &&
    ...
    if (devicePath === '') {
        if (options.debug) {
            console.log(color.yellow('findWheel -> Oops, could not find a G27 Wheel. Is it plugged in?\n'))
            process.exit()
        }
    } else if (options.debug) {
        console.log(color.cyan('findWheel -> Found G27 Wheel at ') + devicePath)
    }
